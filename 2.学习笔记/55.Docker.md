...
用来指定镜像的默认入口命令，该入口命令会在容器启动时作为根命令（即`PID=1`）执行，所有其他传入值作为该命令的参数。

`docker run` 命令的 `--entrypoint` 选项可以覆盖 **Dockerfile** 文件中的 **ENTRYPOINT** 指令；

如果 **Dockerfile** 文件中有多个 **ENTRYPOINT**，则只有最后一个生效；

与 **CMD** 指令的区别在于 **ENTRYPOINT** 用于指定启动父进程（即`PID=1`），可以把会变动的参数写到 **CMD** 指令中，而把那些不需要变动的参数写到 **ENTRYPOINT** 指令中。使用 `docker run` 命令时使用参数来覆盖 **CMD** 指令内容。
...
# 语法2，exec形式
# 会屏蔽掉 docker run 后面指定的参数和 CMD 命令中指定的参数。
