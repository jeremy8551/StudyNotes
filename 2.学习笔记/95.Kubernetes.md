...
官方链接：https://Kubernetes.io/zh-cn/docs/home/
...
Kubernetes 是一个全新的基于容器技术的分布式架构领先方案，是谷歌严格保密十几年的秘密武器----Borg系统的一个开源版本，于2014 年 9 月发布第一个版本，2015 年 7 月发布第一个正式版本。

Kubernetes 的本质是**一组服务器集群**，可以在集群的每个节点上运行特定的程序，来对节点中的容器进行管理。

目的是实现资源管理的自动化，主要提供了以下功能：
...
5. kubelet接收到指令后，会通知docker，然后由docker来启动一个 nginx 的 Pod；

   Pod是Kubernetes的最小操作单元，容器必须跑在Pod中；

6. 至此一个nginx服务就运行了，如果需要访问nginx，就需要通过 kube-proxy 来对 Pod 产生访问的代理；
...
在 Kubernetes 中所有的内容都抽象为资源，学习 Kubernetes 的核心，就是学习如何对集群上的 `Pod`、`Pod控制器`、`Service`、`存储` 等各种资源对象进行操作。
...
只使用 kubectl 命令对 Kubernetes 集群进行管理。



### 命令式对象配置

就是使用 kubectl 命令配合 YAML 配置文件一起来操作 Kubernetes 资源。
...
只通过一个 `apply` 命令和 YAML 配置文件去操作 Kubernetes 资源。

`apply` 命令：如果资源不存在就创建（相当于 kubectl create），如果资源已存在则更新（相当于 kubectl patch）。

```shell
# 首先执行 kubectl apply -f yaml，发现创建了资源
...
资源清单（Resource Manifest）是一种以 YAML 或 JSON 格式编写的文件，用于描述和定义 Kubernetes 中的各种资源对象。

资源清单文件通常包含了一系列的 API 对象定义，每个对象都描述了一个 Kubernetes 资源及其配置。

API 对象是 Kubernetes 中的核心概念，资源清单中的每个条目都是一个 API 对象的定义，包含了该对象的元数据、规范和状态等信息。

资源清单文件中的每个 API 对象通常包含以下三个部分：

1. **Metadata（元数据）**：包含有关对象的信息，例如名称、命名空间、标签、注释等。
2. **Spec（规范）**：描述了对象的所需状态和配置，例如容器的镜像、资源请求、存储卷挂载等。
3. **Status（状态）**：包含了对象的当前状态信息，例如运行状态、可用性等。

通过将资源清单文件中的 API 对象应用到 Kubernetes 集群中，用户可以实现对集群中的资源进行创建、修改、查询和删除等操作。

API 对象和资源清单是 Kubernetes 中进行集群管理和配置的基础。
...
Kubernetes 的最小控制单元，容器都是运行在Pod中的，一个Pod中可以有1个或者多个容器。



## Controller

Pod 控制器，用来管理 Pod，比如启动 Pod、停止 Pod、伸缩 Pod 的数量等。
...
标签，用于对 Pod 进行分类，同一类Pod会拥有相同的标签。
...
如果要**创建**或**更新**资源，就用**声明式对象配置**方式：`kubectl apply -f XXX.yaml`

如果要**删除**资源，就使用**命令式对象配置**方式：`kubectl delete -f XXX.yaml`

如果要**查询**资源，就用**命令式对象管理**方式：`kubectl get 资源名称`
...
其中：
- `POD_NAME` 是要执行命令的Pod的名称；
- `-c CONTAINER` 可选参数，用于指定要在其中执行命令的容器的名称。如果Pod中只有一个容器，可以省略该参数；
- `COMMAND [args...]` 是要在容器内部执行的命令及其参数；
- `-n` 选项指定 Pod 归属的命名空间；
- `-it` 选项指定交互式终端和终端输入（TTY）等；

使用案例：

```bash
# 在名为mypod的Pod中执行date命令
kubectl exec mypod -- date

# 在名为mypod的Pod中的nginx容器中执行ls命令
kubectl exec mypod -c nginx -- ls -l

# 在名为mypod的Pod中的nginx容器中以交互模式进入bash shell
...
在名为 mypod 的 Pod 中的 nginx 容器中以交互模式进入bash shell
...
kubectl 的运行是需要进行配置的，配置文件是 `$HOME/.kube`，如果想要在 Node 节点运行命令，需要将 Master 上的 `.kube` 文件复制到 Node 节点上，即在 Master 节点上执行命令 `scp -r HOME/.kube Node1: HOME/`
...
在 Kubernetes 中命名空间（Namespace）是一种用于将 Kubernetes 资源对象分组和隔离的机制。

在 Kubernetes 中允许用户在同一个集群中创建多个虚拟的、逻辑上隔离的工作环境，每个命名空间可以包含一组特定的资源对象，如 Pod、Service、Deployment 等。
...
Kubernetes 启动后，会默认创建几个 Namespace，如 `default`、`kube-system` 等，同时也允许用户自定义和创建新的命名空间来满足不同的管理和使用需求。
...
在 Kubernetes 中每个 Pod 都会分配一个单独的 IP，可以通过这个 IP 来访问 Pod 中的应用程序，然而却存在如下两问题：

- Pod IP 会随着 Pod 的重建产生变化；
- Pod IP 仅仅是集群内可见的虚拟IP，外部无法访问；

为了解决以上问题，Kubernetes 提供了 Service 资源，Service 会对多个 POD 进行聚合（Service 可以看作是一组同类 Pod **对外的访问接口**），并且提供一个统一的入口地址。通过访问 Service 的入口地址就能访问到后面的Pod服务。
...
当创建 Service 的时候会通过 apiServer 向 etcd 写入创建的 Service 信息，而 kube-proxy 会基于监听的机制发现这种 Service 的变动，然后**会将最新的 Service 信息转换成对应的访问规则**，这样就实现了 Serivce 的功能。
...
Pod 与容器的生命周期可能很短，会被频繁地创建和销毁。那么容器在销毁时，保存在容器中的数据也会被清除。

为了能持久化保存容器中的数据，Kubernetes 引入了 Volume 的概念。

Volume 的生命容器不与Pod中单个容器的生命周期相关，当容器终止或者重启时，Volume 中的数据也不会丢失。

常见的 Kubernetes 的 Volume 类型有：
...
在 Pod 被分配到 Kubernetes 集群中的某一个节点时，由 Kubernetes 自动创建一个空目录，就是 EmptyDir。

当 Pod 销毁时， EmptyDir 中的数据也会被永久删除。 

使用场景：

- 临时空间，例如用于某些应用程序运行时所需的临时目录，且无须永久保留；
- 同一个 Pod 中，一个容器需要从另一个容器中获取数据时，可以使用 EmptyDir 共享数据；
...
可以用 hostPath 将数据持久化到 Pod 所在主机中。

将 Pod 所在主机中一个实际目录挂在到 Pod 中，以供 Pod 中的容器使用。

这样的设计就可以保证即使 Pod 销毁了，但是数据文件可以存在于 Node 主机上。
...
是网络文件存储系统，可以搭建一台 NFS 服务器，然后将NFS 系统上的文件与目录挂载到 Pod 中。

无论 Pod 在 Kubernetes 节点上怎么转移，只要 Node 跟 NFS 的网络没问题，就可以成功访问数据。
...
PV 是持久化卷的意思，是对底层的共享存储的一种抽象。使用了 PV 和 PVC 之后，工作可以得到进一步的细分：

- 存储：存储工程师维护；
- PV： Kubernetes 管理员维护，与底层具体的共享存储技术有关，并通过插件完成与共享存储的对接；
- PVC：Kubernetes 用户维护；
...
- **存储类别**
  ...
  PV 可以通过 storageClassName 选项指定一个存储类别

  - 具有特定类别的 PV 只能与请求了该类别的 PVC 进行绑定；
  - 未设定类别的 P V则只能与不请求任何类别的 PVC 进行绑定
...
PVC 持久卷声明，是用户向 Kubernetes 系统发出的一种资源需求申请（用来声明对存储空间、访问模式、存储类别的需求）。
...
用户创建 PVC，Kubernetes 负责根据 PVC 的声明去寻找 PV，并绑定。
...
当存储资源使用完毕后，用户可以删除 PVC，与该 PVC 绑定的 PV 将会被标记为**已释放**，但还不能立刻与其他 PVC 进行绑定。

通过之前 PVC 写入的数据可能还被留在存储设备上，只有在清除之后该 PV 才能再次使用。
...
对于 PV 管理员可以设定回收策略，用于设置与之绑定的 PVC 释放资源之后如何处理遗留数据的问题。

只有 PV 的存储空间完成回收，才能供新的 PVC 绑定和使用。
...
ConfigMap 用于存储非敏感的明文数据，例如环境变量、配置文件等。

ConfigMap 可以实现将配置数据与 Pod 分离，从而使配置更易于管理和更新。
...
执行上诉命令后 `ConfigMap` 的 `Key-Value` 信息就已经存入了 `etcd` 数据库，后续就可以被其他 `API` 对象使用。



#### 使用命令创建

可以用 `kubectl create configmap` 命令新建 ConfigMap。
...
会创建一个名为 `my-config` 的 ConfigMap，其中包含了两个键值对，分别为 `key1=value1` 和 `key2=value2`。



#### 使用目录创建

假设有一个名为 `config-files` 的目录，其中包含两个文件：`file1.txt` 和 `file2.txt`，内容分别为：
...
使用 `config-files` 目录作为数据源，创建一个名为 `my-config` 的 ConfigMap。

```bash
kubectl create configmap my-config --from-file=config-files
```

通过以上命令，Kubernetes 将会读取 `config-files` 目录中的所有文件，并转换为 ConfigMap 中的键值对。结果如下所示：
...
可以使用特定的文件作为数据源，创建 ConfigMap 资源。

假设有一个目录 `config-files` 包含两个文件：`file1.txt` 和 `file2.txt`，内容分别为：
...
使用文件 `file1.txt` 和 `file2.txt` 作为数据源，创建名为 `my-config` 的 ConfigMap

```bash
kubectl create configmap my-config --from-file=config-files/file1.txt --from-file=config-files/file2.txt
```
...
可以使用环境变量的方式从 Shell 中传递数据来创建 ConfigMap：

```bash
export KEY1=value1
export KEY2=value2
kubectl create configmap my-config --from-literal=key1=$KEY1 --from-literal=key2=$KEY2
```

这将创建一个名为 `my-config` 的 ConfigMap，其中包含两个键值对，分别是 `key1=value1` 和 `key2=value2`。



#### 从 Secret 对象创建

首先创建 Secret 对象 `my-secret`，包含了用户名和密码，且已经通过 base64 编码进行了加密。

然后创建 ConfigMap 对象 `my-config`，从 `my-secret` 对象中获取用户名和密码，并作为键值对存储在 ConfigMap 中。
...
metadata:
  name: my-secret
data:
  username: YWRtaW4=   # 使用 base64 编码的用户名
  password: MTIzNDU2   # 使用 base64 编码的密码
---
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  username: |-
    {{- .Data.username | b64dec }}
  password: |-
    {{- .Data.password | b64dec }}
```

在这个示例中，使用了 Go 模板语法 `{{- .Data.username | b64dec }}` 和 `{{- .Data.password | b64dec }}` 来从 Secret 对象中提取 base64 编码的用户名和密码，并将其解码后存储到 ConfigMap 中。
...
新建 ConfigMap 资源对象，再将已创建的 ConfigMap 资源作为 Pod 中的 Volume 卷，将 Volume 卷挂载到容器中指定目录/文件。

这时已将 ConfigMap 中的 key 映射为容器中的文件名，将 value 映射为文件内容。将 Volume 挂载到容器后，如果更新 ConfigMap 资源中的内容，容器中的值也会动态更新。
...
执行 `configmap.yaml` 文件新建 ConfigMap 资源对象
...
创建 `Pod-configmap.yaml` 将 configmap 挂载到容器中
...
[root@k8s-Master01 ~] # cd /configmap/config/
[root@k8s-Master01 ~] # ls
info
[root@k8s-Master01 ~] # more info
username:admin
password:123456
```

已将 ConfigMap 中的 key 映射为容器中的文件 `/configmap/config/info`，将 value 映射为文件内容。



#### 作为环境变量

可以通过环境变量的方式将其中的数据注入到容器内部。

[使用案例详见链接](######configMapKeyRef)



## Secret

用于存储需要加密的信息，例如密码、秘钥、证书等等。
...
type: Opaque
data:
  username: {{ .Files.Get "my-secret-data/username.txt" | b64enc }}
  password: {{ .Files.Get "my-secret-data/password.txt" | b64enc }}
```

在这个 YAML 文件中，定义了一个名为 `my-secret` 的 Secret 对象，它包含了两个数据项：`username` 和 `password`。

使用 `{{ .Files.Get "my-secret-data/username.txt" | b64enc }}` 和 `{{ .Files.Get "my-secret-data/password.txt" | b64enc }}` 从文件中获取数据，并使用 base64 编码存储到 Secret 中。



#### 使用命令创建

想要创建一个基本认证类型的 Secret，其中包含用户名和密码。

首先需要将用户名和密码进行 base64 编码：
...
这将分别输出用户名和密码的 base64 编码值。

使用命令来创建一个名为 `my-secret` 的通用（generic）类型的 Secret 对象：

```shell
kubectl create secret generic my-secret --from-literal=username=YWRtaW4= --from-literal=password=cGFzc3dvcmQ=
```

在这个命令中，使用长选项 `--from-literal` 来设置键值对信息。



#### 使用环境变量创建

创建一个名为 `my-secret` 的 Secret 对象，并将用户名和密码的值分别存储到 `my-secret` Secret 对象的 `username` 和 `password` 字段中。
...
有两个文件，`username.txt` 和 `password.txt`，分别包含了用户名和密码的值。
...
使用命令使用这些文件创建一个名为 `my-secret` 的 Secret 对象：

```bash
kubectl create secret generic my-secret --from-file=username.txt --from-file=password.txt
```

可以使用命令来查看 Secret 对象的详细信息：
...
使用以下命令创建一个名为 `my-configmap` 的 ConfigMap 资源对象：

```bash
kubectl create configmap my-configmap --from-literal=username=admin --from-literal=password=P@ssw0rd!
```

然后使用以下命令从 `my-configmap` ConfigMap 资源对象创建一个名为 `my-secret` 的 Secret 对象：

```bash
kubectl create secret generic my-secret --from=configmap/my-configmap
```

这条命令将会从现有的 `my-configmap` ConfigMap 资源对象中读取数据，并将其存储到新创建的 `my-secret` Secret 对象中。



#### 从Secret创建

使用以下命令创建一个名为 `existing-secret` 的 Secret 资源对象：

```shell
kubectl create secret generic existing-secret --from-literal=username=admin --from-literal=password=P@ssw0rd!
```

创建一个新的名为 `my-secret` 的 Secret 对象，并将其数据存储到新的 Secret 对象中。

```bash
kubectl create secret generic my-secret --from secret=existing-secret
```
...
Secret 中的数据可以通过环境变量注入到容器中，容器可以直接从环境变量中读取敏感数据。

这样的好处是可以方便地在容器启动时注入敏感数据，而不需要在容器内部进行处理。

[点击链接查看详情](######secretRef)



#### 配置 ServiceAccount 访问权限

首先创建一个 Secret 对象，用于存储需要的访问凭证：
...
将 `BASE64_ENCODED_USERNAME` 替换为用户名的 base64 编码内容。

将 `BASE64_ENCODED_PASSWORD` 替换为密码的 base64 编码内容。

然后创建一个 ServiceAccount 对象，并将上面创建的 Secret 对象挂载到该 ServiceAccount：
...
最后将 ServiceAccount 与需要访问的资源进行绑定，例如 Pod、Deployment 或者其他资源对象：
...
将 ServiceAccount `my-serviceaccount` 绑定到了名为 `my-clusterrole` 的 ClusterRole 上，以赋予其对应资源的访问权限。
...
Secret 还可以用来存储 TLS 证书和私钥，用于配置 HTTPS 服务。

在 Ingress、Service、Pod 等对象中使用 Secret 来配置 TLS 证书，实现加密通信。

使用案例：

首先要准备 TLS 证书和私钥文件，假设文件名分别为 `tls.crt` 和 `tls.key`。

创建一个 Secret 对象，将 TLS 证书和私钥存储在其中：
...
data:
  tls.crt: BASE64_ENCODED_CERTIFICATE_CONTENT
  tls.key: BASE64_ENCODED_PRIVATE_KEY_CONTENT
```

在上面的 YAML 文件中，将 `BASE64_ENCODED_CERTIFICATE_CONTENT` 替换为 `tls.crt` 文件的 base64 编码内容，将 `BASE64_ENCODED_PRIVATE_KEY_CONTENT` 替换为 `tls.key` 文件的 base64 编码内容。
...
`secretName` 字段引用了之前创建的 `tls-secret` Secret 对象，用于配置 TLS 证书。
...
这样就完成了 TLS 证书的配置，Ingress 会使用指定的 TLS 证书与客户端进行加密通信。
...
所谓的安全性其实就是保证对 Kubernetes 的各种**客户端**进行**认证和鉴权**操作。
...
- **Service Account**：Kubernetes 管理的账号，用于为Pod中的服务进程在访问Kubernetes时提供身份标识。
...
API 对象是 Kubernetes 中的核心概念之一，用于定义和管理集群中的资源，以及为运行在集群中的应用程序提供配置和状态信息。

使用 API 对象创建的就是 Kubernetes 资源对象（包括 Pod、Service、Deployment、ConfigMap、Secret 等）。

每个 API 对象都有一个与之相关的 API 版本（例如 `v1` 版本）和类型（例如 `Pod`、`Service`、`Deployment` 等）。

API 对象通常由以下三个部分组成：

1. **Metadata（元数据）**

   元数据包含有关对象的信息，例如名称、命名空间、标签、注释等；

   这些信息帮助 Kubernetes 系统和用户识别和管理对象；

2. **Spec（规范）**

   规范部分描述了对象的所需状态；

   例如，对于一个 Pod 对象，规范可能包括容器镜像、资源请求、挂载的存储卷等配置信息；

3. **Status（状态）**

   状态部分包含了对象的当前状态信息；

   例如：对于一个 Pod 对象，状态可能包括容器的运行状态、就绪状态、IP 地址等信息；



## apiVersion

Kubernetes 默认提供了很多 API 组，每个 API 组都包含了一组特定类型的资源，用于管理 Kubernetes 集群的不同方面。

`apiVersion` 选项可以指定 Kubernetes 资源对象使用的 API组和版本（类似于 Java 中的 import 语句）。

`apiVersion` 格式是 `<group>/<version>` 其中 `<group>` 表示 API 分组，`<version>` 表示 API 分组的版本。

常用的 API 组：

1. **core**

   Kubernetes 的核心 API 组，包含了最基本的资源类型，如 Pod、Service、Namespace、PersistentVolume 等；

   如下所示，如果 `apiVersion` 中没有指定 `group` 的值，则该资源对象使用 core API 组；
...
2. **apps**
   ...
   这个 API 组包含了与应用程序部署和管理相关的资源，如 Deployment、ReplicaSet、StatefulSet 和 DaemonSet 等；

   ```yaml
   apiVersion: apps/v1
   ```

   表示正在使用 Kubernetes 的 `apps` API 组的第一个版本；

3. **extensions**

   这个 API 组包含了一些扩展资源，比如 Ingress；

4. **batch**

   这个 API 组包含了与批处理作业相关的资源，如 Job 和 CronJob；

5. **rbac.authorization.k8s.io**

   这个 API 组包含了与访问控制相关的资源，如 Role、RoleBinding、ClusterRole 和 ClusterRoleBinding；

6. **networking.k8s.io**

   这个 API 组包含了与网络相关的资源，如 NetworkPolicy；

7. **storage.k8s.io**

   这个 API 组包含了与存储相关的资源，如 StorageClass；

8. **autoscaling**

   这个 API 组包含了与自动扩展相关的资源，如 HorizontalPodAutoscaler；

9. **authentication.k8s.io** 和 **authorization.k8s.io**

   这两个 API 组包含了与身份验证和授权相关的资源，如 TokenReview、SubjectAccessReview 等；

10. **apiextensions.k8s.io**

    这个 API 组包含了与自定义资源和自定义资源定义相关的资源，如 CustomResourceDefinition；

也可以使用如下命令查看所有可用的 API：
...
在 Kubernetes 中 `metadata` 是用于描述资源对象元数据的选项，包含了一些关于资源对象的基本信息和属性。
...
一组键值对，用于对资源对象进行分类和标记，通常用于选择和筛选资源对象。
...
有三个 Pod，签分别是 `app: my-app` 和 `environment: production`。
...
如果想选择带有标签 `app: my-app` 和 `environment: production` 的所有 Pod，可以使用以下标签选择器：
...
annotations（注解）是一组键值对，用于存储额外的信息，以供用户或工具使用。

注意 Labels 与 annotations 不同，Labels 用于标识资源对象自身属性，用于筛选和查找对象等。

Annotations 可以在创建对象时指定，也可以随时添加、修改或删除。

Annotations 可用于描述对象的特定配置、版本信息、审计日志、监视、调试和管理 Kubernetes 集群中的对象。
...
是 Service 资源对象的一个字段，用于指定服务的集群内部 IP 地址。

当创建 Service 时，可指定一个固定的集群内部 IP 地址，该 Service 可以被 Pod 或其他 Service 访问。

如果不指定 `spec.clusterIP`，Kubernetes 将为 Service 分配一个动态 IP 地址。
...
当 Service 的 `spec.clusterIP` 设置为 `None` 时，表示是一个 headless Service（无集群IP）。

headless Service 不会分配集群内部的 IP 地址，能通过 DNS 来方式。

headless Service 通常用于以下情况：

1. **StatefulSets**

   在 StatefulSets 中，每个 Pod 都有一个唯一的标识符（例如 Pod 名称），可通过 DNS 查询来访问。

   在这种情况下，创建一个无集群 IP 的 Service 可以让每个 Pod 使用其唯一标识符来进行网络通信。

2. **外部服务代理**

   当希望 Service 只作为其他服务的代理，而不提供自己的 IP 地址时，可以将 `spec.clusterIP` 设置为 `None`。

   例如：可能有一个外部的负载均衡器或代理，会转发流量到 Service 所代理的后端服务。
...
1. **metadata**：包含 Pod 的元数据，例如名称、标签等。
   ...
   `template.metadata.labels` 用于定义资源对象模板中的 Pod 模板的标签，而 `metadata.labels` 用于定义资源对象自身的标签；
...
可以设置 Pod 的 DNS 解析策略，如果 dnsPolicy 字段未设置，默认策略是 `ClusterFirst`。
...
首先尝试在集群中解析 DNS，如果找不到，则使用节点上的 DNS 配置进行解析。
...
与 ClusterFirst 类似，但在主机网络模式下使用时，Pod 使用节点上的 DNS 配置进行解析。



#### None

禁用 DNS 解析功能。在这种情况下，Pod 必须配置自己的 DNS 解析功能。通常与 hostNetwork 一起使用，以确保 Pod 可以访问主机上的网络。



#### DefaultWithHostNet

与 Default 类似，但在主机网络模式下使用时，Pod 使用节点上的 DNS 配置进行解析。
...
控制器对象的更新策略，默认的更新策略是 RollingUpdate。

如果更新了 Deployment 的 Pod 模板部分（即 `spec.template`），例如更改容器的镜像版本或配置，那么 Deployment 将会触发更新，以将新的模板应用到现有的 Pod 副本上。
...
在 Kubernetes 中，`spec.containers` 用于定义 Pod 中的容器，是一个包含一个或多个容器定义的列表。
...
比如 **busybox** （不是一个程序）是类似于一个工具类的集合，容器启动后会自动关闭。
...
可以为 Pod 中的容器设置环境变量，环境变量可以在容器内部访问，用于传递配置信息、认证凭据、服务地址等。

下面是在 `env` 字段中常见的选项，且支持从不同的资源中获取值，使设置环境变量的值更加动态和灵活。
...
不推荐这种方式，可以将这些配置单独存储在 ConfigMap 配置文件中。



##### valueFrom

从其他资源（例如 Secret 或 ConfigMap）中引用值。可以使用以下子选项：



###### secretKeyRef

从 Secret 中引用值，包含 name（Secret 的名称） 和 key（Secret 中包含要使用的值的密钥名称） 两个选项。
...
可以将环境变量存储在 ConfigMap 对象中，并在环境变量中引用这些值。

子选项有：name（ConfigMap 的名称）和 key（ConfigMap 中包含要使用的值的键名称） 是两个子选项。
...
从 Pod 的字段中引用值，有 2 个子选项：



**fieldPath**

值是字符串，用于设置要从 Pod 中引用字段的路径；
...
1. **直接字段名称**
   ...
   例如 `metadata.name` 表示获取资源对象的 metadata 字段中的 name 字段的值；

2. **子字段索引**

   如果字段是一个数组，可以使用索引来获取其中一个元素的值；

   例如，`spec.containers[0].name` 表示获取容器数组中第一个容器的 name 字段的值；

3. **递归字段**

   可以通过`.`符号来表示嵌套字段。

   例如，`metadata.ownerReferences[0].kind` 表示获取资源对象的 metadata 字段中的 ownerReferences 数组的第一个元素的 kind 字段的值。

4. **通配符**

   可以使用 `*` 通配符来获取所有匹配的字段的值；

   例如，`spec.*` 表示获取 spec 字段中的所有子字段的值；

5. **`[]` 语法**

   可以使用 `[]` 语法来表示一个包含多个字段的数组；

   例如，`status.conditions[*].type` 表示获取 status 字段中的 conditions 数组中所有元素的 type 字段的值；



**apiVersion**

可选项，指定要引用的 Kubernetes API 版本；
...
从资源字段中引用值，例如引用 CPU 和内存数量，包含如下子选项：

1. containerName：要引用哪个容器（名称）中的资源；

2. resource：资源路径，例如 `limits.cpu`；

3. divisor：可选项，用于指定资源的除数；

  如果 CPU 限制值为 2000m（即 2 核）、divisor 值为 1000，则经过除法运算后，环境变量的值将为 2；
  ...
  - name: my-container
    ...
    resources:
      limits:
        ...
        memory: "2Gi"
    env:
			- name: CPU_LIMIT_MCORE
  		  valueFrom:
    			resourceFieldRef:
        		containerName: my-container
			      resource: limits.cpu
      			divisor: "1000" # 将容器的 CPU 限制值直接除以 1000
```
...
引用 ConfigMap 对象中所有的键值对作为环境变量，而 `configMapKeyRef` 只可引入某一个键值对。

包含一个子选项 `name`（ConfigMap 的名称）。
...
下面可以使用 `configMapRef` 将整个 ConfigMap 对象引入到容器环境变量中。
...
引用 Secret 对象中的所有键值对作为环境变量，而 `secretKeyRef` 只引用 Secret 中指定键的值。

有 2 个子选项：

1. **name**：指定要引用的 Secret 的名称。
2. **optional**：可选的布尔值（默认 false），指示是否允许引用的 Secret 不存在，如果设置为 true，则表示允许引用的 Secret 不存在，此时会将环境变量的值设为 null；
...
可以限制资源对象使用的 CPU 数量与内存大小，有以下 2 个子选项：



##### limits

容器最多能占用的资源，超过限制时会被终止，并进行重启。



###### cpu

> 在计算机领域，CPU（中央处理器）和核心（Core）是两个相关但不同的概念：
>
> 比如一台计算机，配备了一块四核心的 CPU，CPU 是一个处理器，而每个核心都是 CPU 中的物理处理单元。

表示 CPU 的核心数，如下所示，单位不同，值的意识也不同。

- 如果值是一个整数，例如 `2`，那么这个值表示容器可以使用的 CPU 核心数为 2 核；

- 如果值是一个小数，例如 `0.5`，那么这个值表示容器可以使用的 CPU 核心数为 0.5 核（即一半的核心）；

- 如果单位是毫核（millicores），如 `500m`，那么表示容器可以使用的 CPU 核心数为 500 毫核，相当于 0.5 核；

  `2000m` 表示 2 核；

  在 Kubernetes 中，CPU 的计算单位为 `millicores`（毫核），表示 CPU 资源的千分之一，"1 核" 等于 "1000 毫核"；
  
  使用毫核单位可以更精细地控制容器对 CPU 资源的使用；
...
用于指定 PersistentVolumeClaim（PVC）对象的存储容量的大小，以确保在调度 Pod 时可以为其分配足够的存储资源。

单位可以是字节（`B`）、千字节（`KB`）、兆字节（`MB`）、吉字节（`GB`）等。例如，`1Gi` 表示请求 1GB 的存储容量。
...
创建了一个名为 `myclaim` 的 PVC 资源，并在其中指定了存储资源的请求量为 1GB（`1Gi`）。
...
`securityContext: {}` 表示使用默认的安全设置（容器将使用默认的 Linux 安全选项、非特权模式等）。
...
Volumes 用于将存储介质（如存储卷、宿主机文件系统、网络存储等）挂载到容器中，从而为容器提供持久化的存储空间。

Volumes 允许容器之间共享数据，并提供了一种方便的方式来处理容器中的数据持久化和共享。
...
存储卷的名称，用于在 Pod 中标识该存储卷的唯一标识符，与容器中 `volumeMounts.name` 属性值对应。



#### emptyDir

用于在 Pod 中创建一个空目录，并在该 Pod 的所有容器之间共享这个空目录，但在 Pod 生命周期结束时，其中的数据将会被清除。
...
用于将 Pod 所在主机上的特定路径挂载到 Pod 中的容器中。
...
将 Pod 主机上的目录 `/path/on/host` 挂载到容器中的 `/host-path` 目录上。



##### path

主机文件系统中的目录路径，确保是主机文件系统中的有效路径，并且有足够权限。

支持格式如下：

1. **绝对路径**

   指定完整的主机文件系统路径，例如 `/var/log`、`/etc/config` 等；

2. **相对路径**

   相对于主机文件系统的当前工作目录的路径；

   这种写法适用于在 Pod 中的容器中执行时，当前工作目录与主机文件系统中的路径相对应；

   例如，`logs` 表示当前工作目录下的 `logs` 目录；

3. **路径包含通配符**

   可以使用通配符来指定一类文件或目录；

   例如：`/var/log/*.log` 表示 `/var/log` 目录下的所有 `.log` 文件；

4. **环境变量引用**

   可以使用环境变量来引用主机文件系统中的路径；

   例如：`$(HOME)/config` 表示主机用户根目录下的 `config` 目录；

5. **命名卷**

   如果在集群中创建了命名卷，并将其绑定到主机路径，可以在 `path` 字段中引用该命名卷。

   例如，`/mnt/data` 表示绑定到 `/mnt/data` 的命名卷；
...
如果指定的路径不存在，则会在主机上创建一个目录。

如果路径已经存在并且是一个目录，则将其挂载到 Pod 中。



###### FileOrCreate

如果指定的路径不存在，则会在主机上创建一个文件。

如果路径已经存在并且是一个文件，则将其挂载到 Pod 中。
...
通过 PersistentVolumeClaim（PVC）来声明持久化存储，通常与持久卷（PersistentVolume）结合使用。

使用案例：

```yaml
# 创建 PersistentVolumeClaim（PVC）对象
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: myclaim
...
- 首先创建名为 `myclaim` 的 PersistentVolumeClaim（PVC）对象（1Gi 存储容量），并指定了 `ReadWriteOnce` 的访问模式；
- 然后创建名为 `mypod` 的 Pod 对象，并定义了名为 `pvc-volume` 的 Volume，类型为 PersistentVolumeClaim；
- 最后将 `myclaim` PVC 挂载到了 Pod 中的 `/data` 目录下，以供容器中的应用程序使用；
...
1. **ReadWriteOnce (RWO)**
   ...
   该卷只能被单个 Pod 挂载为读写模式，其他 Pod 不能同时挂载该卷；

2. **ReadOnlyMany (ROX)**

   多个 Pod 可以同时挂载该卷，但只能以只读模式进行访问；

3. **ReadWriteMany (RWX)**

   多个 Pod 可以同时挂载该卷，并且可以以读写模式进行访问；



#### configMap

从 ConfigMap 对象中挂载配置数据。
...
- 首先创建 `myconfigmap` 的 ConfigMap 对象，并定义了一个名为 `config.yaml` 的数据项，其中包含了一些配置数据；
- 然后创建了一个名为 `mypod` 的 Pod 对象，并定义了一个名为 `config-volume` 的 Volume，类型为 ConfigMap；
- 将 `myconfigmap` ConfigMap 挂载到了 Pod 中的 `/etc/config` 目录下，并将其重命名为 `my-config.yaml`；



#### secret

从 Secret 对象中挂载敏感数据，如密码、令牌等。
...
- 首先创建了一个名为 `mysecret` 的 Secret 对象，并在其中定义了一个 `username` 和一个 `password`；
- 然后创建了一个名为 `mypod` 的 Pod 对象，并定义了一个名为 `secret-volume` 的 Volume，类型为 Secret；
- 将 `mysecret` Secret 挂载到了 Pod 中的 `/etc/secret` 目录下，以供容器中的应用程序使用；



#### nfs

从 NFS（Network File System）服务器中挂载网络存储。
...
spec:
  ...
  - name: nfs-volume
    nfs:
      ...
      path: /exports/data      # NFS 服务器上的共享目录路径
      readOnly: true           # 默认值为 false，表示 NFS 卷是可读可写的
                               # 如果设置为 true，则 NFS 卷将以只读方式挂载到容器中
```

在这个案例中：

- 创建了一个名为 `mypod` 的 Pod 对象，并在其中定义了一个名为 `nfs-volume` 的 Volume，类型为 NFS；
- 将 NFS 服务器地址设置为 `nfs.example.com`，并将 NFS 服务器上的共享目录路径设置为 `/exports/data`；
- 然后将 NFS 卷挂载到了 Pod 中的 `/mnt/nfs` 目录下，以供容器中的应用程序使用；
...
将 Google Compute Engine 持久磁盘挂载到容器中。



#### awsElasticBlockStore

将 AWS Elastic Block Store 挂载到容器中。
...
每次对 Pod 控制器进行更新（例如更改 Pod 模板或更新标签选择器）都会创建一个新的 ReplicaSet，并保留之前的 ReplicaSet 以用于回滚操作，这个字段（整数类型）用于控制保留的旧 ReplicaSet 的最大数量，以限制集群中 ReplicaSet 对象数量。如果新创建的 ReplicaSet 的数量超过了指定的限制，则较早的 ReplicaSet 将被自动删除。
...
Pod 控制器（如 Deployment、StatefulSet 等）的更新策略。

可以配置和禁用掉自动滚动更新 Pod 的容器、标签、资源请求或限制、以及注解。
...
当 StatefulSet 的 `.spec.template` 有变动时，StatefulSet 控制器将不会自动更新 Pod，必须手动删除 Pod 以后，控制器会创建新的 Pod。
...
StatefulSet 控制器会删除和重建每个 Pod，将按照与 Pod 终止相同的顺序（从最大序号到最小序号）进行，每次更新一个 Pod。

Kubernetes 控制平面会等到被更新的 Pod 进入 Running 和 Ready 状态，然后再更新其前身。 

如果设置了 `.spec.minReadySeconds`（查看最短就绪秒数）， 控制平面在 Pod 就绪后会额外等待一定的时间再执行下一步。



###### partition

分区滚动更新策略，如果声明了一个分区，当 StatefulSet 的 `.spec.template` 被更新时， 所有序号大于等于该分区序号的 Pod 都会被更新。 所有序号小于该分区序号的 Pod 都不会被更新，并且，即使它们被删除也会依据之前的版本进行重建。 如果 StatefulSet 的 `.spec.updateStrategy.rollingUpdate.partition` 大于它的 `.spec.replicas`，则对它的 `.spec.template` 的更新将不会传递到它的 Pod。 在大多数情况下，你不需要使用分区，但如果你希望进行阶段更新、执行金丝雀或执行分阶段上线，则这些分区会非常有用。



###### maxUnavailable

字段处于 Alpha 阶段，仅当 API 服务器启用了 `MaxUnavailableStatefulSet` 才起作用。

控制更新期间不可用的 Pod 的最大数量。 该值可以是绝对值，或者是期望 Pod 个数的百分比（例如，`10%`）。 

绝对值是根据百分比值四舍五入计算的。 该字段不能为 0。默认设置为 1。
...
提供了一段缓冲时间，确保 StatefulSet 控制器在给定的秒数内处于 `Ready`， 然后才会将该 Pod 报告为 `Available`。 
...
Dashboard 是一个WEB用户界面，提供了 kubectl 命令的绝大部分功能。

可以用来部署容器化的应用，还可以监控应用的状态，执行故障排查以及管理 Kubernetes 中各种资源。



### PodSecurityPolicy

在 Kubernetes v1.21 中被弃用， 在 Kubernetes v1.25 中被移除。
...
在 Deployment 上点击规模，然后指定**目标副本数量**，点击确定
