...
- `kubectl config use-context <context_name>`：切换到特定的上下文。
...
Kubernetes 资源对象的配置文件通常采用 YAML 或 JSON 格式编写，用于描述资源对象的元数据、规范和其他相关信息。

Kubernetes 资源对象的配置文件包含了 Kubernetes 对象的各种属性，如名称、标签、副本数、镜像、容器端口等。

通过将这些配置文件应用到 Kubernetes 集群中，可以创建、修改和管理各种类型的 Kubernetes 资源对象。

这些配置文件可以手动编写，也可以由开发人员使用工具或模板生成。一旦配置文件被创建，可以使用 `kubectl apply` 命令将其应用到 Kubernetes 集群中，使集群根据配置文件中的定义创建相应的资源对象。

不同类型的 Kubernetes 资源对象具有不同的属性。
...
这种方式不是很推荐，推荐将这些配置单独存储在 ConfigMap 配置文件中，这种方式将在后面介绍。
...
ConfigMap 是 Kubernetes 中用于存储非敏感配置数据的一种资源对象，可以存储键值对、文件内容等。
